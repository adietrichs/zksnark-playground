    ...
    def trusted_setup(self):
        ...
        variable_consistency_polynomials = {
            variable: prod(
                [
                    (
                        encrypted_variable_polynomial_dict.get(variable)
                        or group.init(G1, 0)
                    )
                    ** beta
...
class Prover:
    def __init__(self, problem, proving_key):
        ...
        (
            ...
            self.variable_consistency_polynomials,
        ) = proving_key
    ...
    def prove(self, variable_assignments):
        ...
        g_Z = prod(
            variable_consistency_polynomial ** variable_assignments[variable]
            for variable, variable_consistency_polynomial in self.variable_consistency_polynomials.items()
        )
        return [g_L, g_R, g_O, shifted_g_L, shifted_g_R, shifted_g_O, g_h, g_Z]
...
class Verifier:
    def __init__(self, problem, verification_key):
        self.problem = problem
        (
            self.encrypted_alphas,
            self.g_t,
            self.encrypted_shifted_betas,
            self.encrypted_gamma,
        ) = verification_key

    def verify(self, proof):
        [g_L, g_R, g_O, g_L_prime, g_R_prime, g_O_prime, g_h, g_Z] = proof
        ...
        assert pair(g_L, self.encrypted_shifted_betas[0]) * pair(
            g_R, self.encrypted_shifted_betas[1]
        ) * pair(g_O, self.encrypted_shifted_betas[2]) == pair(
            g_Z, self.encrypted_gamma
        )
