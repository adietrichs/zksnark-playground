...
class Problem:
    ...
    def trusted_setup(self):
        ...
        encrypted_betas = [self.g ** beta for beta in betas]
        proving_key = [
...
class Verifier:
    def __init__(self, problem, verification_key):
        self.problem = problem
        self.encrypted_alphas, self.g_t, self.encrypted_betas = verification_key
    ...
    def verify(self, proof):
        ...
        assert pair(g_L, self.encrypted_betas[0]) * pair(
            g_R, self.encrypted_betas[1]
        ) * pair(g_O, self.encrypted_betas[2]) == pair(g_Z, self.problem.g)
